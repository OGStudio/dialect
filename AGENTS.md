...
- **`out` (final concatenation)**: `SwiftContext.out` = `otherBase64ToString(SWIFT_EMB64_CORE) + c.outFields + c.outStructs + c.outContexts + c.outShoulds`. This ordering (GENERATED core first, then F, structs, contexts, should funcs last) means all target code EXCEPT the register/set/oneliners funcs is fully generated — `ver-mac/src/dialect.swift` is the only generator source file. The `out` should also declares `path` (Swift output path) resolved from `outputPaths` matching `SWIFT_TYPE`.
