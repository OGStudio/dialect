...
- **`outSets` (v4-specific, parallel to `outShoulds`)**: `SwiftContext.outSets` holds the generated *`<component-prefix>Set()` function text* (e.g. `cliSet`/`swiftSet`/`ymlSet`/`rootSet`), produced by the `outSets` should: when `c.recentField == F.entities`, `c.outSets = swiftSets(c.entities)` and `recentField` becomes `F.outSets`. Concatenated into `out` after `outContexts` and before `outShoulds`, so Set funcs ARE generated into `dialect.swift` (no hand-written mirror in tmp). `swiftSets` (in `components/swift/swiftFun.swift`, 1 param: `entities [String]`) filters entities by `hasSuffix(SWIFT_SUFFIX_COMPONENT)` ("Component") and calls `swiftSet(entity)` per component. Func name = `prefix + "Set"` where `prefix` = contextName minus `'Context'` lowercased (e.g. `CLIComponent` → `cliSet`). Template: `SWIFT_SET_T` (`%FUNC%`/`%COMPONENT%`). Registers nothing; effects/components call these generated funcs directly.
- **`out` (final concatenation)**: `SwiftContext.out` = `otherBase64ToString(SWIFT_EMB64_CORE) + c.outFields + c.outStructs + c.outContexts + c.outSets + c.outShoulds`. This ordering (GENERATED core first, then F, structs, contexts, Set funcs, should funcs last) means all target code EXCEPT the register/oneliner funcs is fully generated — `ver-mac/src/dialect.swift` is the only generator source file. The `out` should also declares `path` (Swift output path) resolved from `outputPaths` matching `SWIFT_TYPE`.
