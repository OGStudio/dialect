...
- **`outStructs` (v4-specific, separate from `out`)**: `SwiftContext.outStructs` holds the generated *struct-declaration text*, produced by the `outStructs` should: when `c.recentField == F.entityFieldTypes`, `c.outStructs = swiftStructs(c.entities, c.entityFields, c.entityFieldTypes)` and `recentField` becomes `F.outStructs`. It is NOT concatenated into `out` — `out` = `otherBase64ToString(SWIFT_EMB64_CORE) + c.outFields` only (verbatim). The `swiftStructs` fun (in `components/swift/swiftFun.swift`, 3 params: `entities [String]`, `entityFields [Int:[String]]`, `entityFieldTypes [Int:[Int:String]]`) is still a **stub** returning a placeholder — real struct generation is NOT yet implemented (do not "complete" it with fake `//generated` output; it's intentionally left incomplete).
